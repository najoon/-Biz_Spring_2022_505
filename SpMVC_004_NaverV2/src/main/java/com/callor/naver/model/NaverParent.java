...
public class NaverParent<VO> {
	 ...
	 * 이때 RestTemplate 를 사용하여 데이터를 가져오눈데
	 * RestTemplate 은 VO 를 감싸는 List 변수가 한개 필요하다
	 * 그런데 List  를 감싸는 VO가 News, Books,Movies 들이 각각 다르다
	 * 이럴때 parent 클래스를 각가가의 VO 를 감싸는 타입으로
	 * 복수 정의해야 하는 문자가 있다
	 * 이때로 Generic 의 기능을 사용하여 클래스 선언문에 Generic을 정의해두고 이클래스를 사용할때 확정된 VO 를 부착하여 
	 * 사용한다
	 */
