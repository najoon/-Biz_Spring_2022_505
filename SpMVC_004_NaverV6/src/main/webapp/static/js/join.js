  ...
  */
  username?.addEventListener("blur", (e) => {
    ...
    fetch(`${rootPath}/user/idcheck/${current.value}`)
      .then((res) => res.text())
      .then((result) => {
        //서버에서 OK 문자열을 보내면 체크한
        if (result === "OK") {
          const msg = "사용 가능한 USER NAME입니다";
  ...
  password?.addEventListener("input", (e) => {
    ...
    if (passRule2.test(current.value)) {
      ok_msg(error, "비밀번호가 맞음");
    } else {
      error_msg(
        error,
        "비밀번호가 규칙이 맞지 않습니다(특수,영문, 숫자포함 8~15)"
      );
    }
  });
  ...
  re_password?.addEventListener("input", (e) => {
    ...
    if (current.value === password?.value) {
      ok_msg(error, "비밀번호 확인이 일치");
    } else {
      error_msg(error, "비밀번호 확인이 일치하지 않음");
    }
  }); //end re_password
  ...
  email?.addEventListener("input", (e) => {
    ...
    if (emailRule.test(current.value)) {
      ok_msg(error, "Email 형식이 일치");
    } else {
      ok_msg(error, "Email 형식을 확인해 주세요");
  ...
  btn_join?.addEventListener("click", (e) => {
    if (username.value === "") {
      alert("user name  은 반드시 입력");
      username.focus();
      return false;
    }

    if (username.value.length > 20) {
      alert("user name 은 20자 이내로 입력");
      username.focus();
      return false;
    }
