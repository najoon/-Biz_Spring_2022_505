document.addEventListener("DOMContentLoaded", () => {
  ...
  // inputList 0 번요소부터 4번요소까지 form tag에 작성된 순서대로 담겨있다
  ...
  const username_isBlank = () => {
    ...
    /*
    js 코드를 사용하여 여러가지 style 을 변경하고 할대 일일이 style 항목을 변결하는 방법이 있지만
    미리 본문의 style 항목에 사용하지 않는 class 하나 만들어 두고
    속성을 미리 작성해 놓는다
    그리고 해당 항목에 classList.add("class명")과 같이 사용하면 여러가지 미리 설정한 속성이 적용된다
    */
  ...
  if (id_check) {
    id_check.addEventListener("click", () => {
      ...
      //queryString 방식으로 요청
      //localhst:8080/school/user/idcheck?username=callor
      //URL(주소창)에 서버보내는 변수이름(username)이 노출되는 문제가 있다
      // 보내는 데이터의 길이 제한이 있다

      // URL,PathVarriable 방식으로 데이터(검사하고자 하는 username) 를 전송하기
      // REST Ful 방식이라고 한다
      // localhost:8080/school/user/idcheck/callr 로 요청하기
