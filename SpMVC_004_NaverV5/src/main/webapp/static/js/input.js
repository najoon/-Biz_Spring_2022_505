document.addEventListener("DOMContentLoaded", () => {
  ...
  //div.modal-result 에 포함된 요소중에서 div.search-result 를 가져와라
  ...
  // 정규식 문법
  // 문자열 중에서 태그 문자열을 무조건 삭제(제거) 하기 위한 패턴
  ...
  modal_box.addEventListener("click", (tb) => {
    ...
    const td = tb.target;
    if (td.tagName === "TD") {
      // td 를 감싸고 있는 가장 가까운 영역의 tr 를 추출하기
      const tr = td.closest("TR");
      // 추출된 tr 의 속성중에 data- isbn 으로 설정된 값을 추출하기
