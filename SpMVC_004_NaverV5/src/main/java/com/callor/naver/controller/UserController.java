...
public class UserController {
	...
	public String join(Model modal) {
		modal.addAttribute("LAYOUT", "JOIN");
		return "home";
	...
	public String login(String error, Model modal) {
		modal.addAttribute("error", error);
		modal.addAttribute("LAYOUT", "LOGIN");
		return "home";
	 ...
	 * Controller 의 각 method 는 Spring 으로부 터 httpSession 객체 값을 주입받을수 있다
	 * HttpSession 객체에는 Clinent 와  server 간에 상테 를 유지하는 정보를 담을수 있다
	 * 여기에 담긴 데이터는 서버가 run 하는 동안 일정 조건을 만족하는 동안 Session 을 제거하지 않는한 계속 메모리에 값이 남아있다
	 * 큰 데이터를 httpSession 에 담는것은 썩 좋지 않다
	 * 최소한으로 status 를 유지하는 데이터만 담는 것이 좋다
	 * 필요가 없어지면 반드시 remove 해주는 것이 좋다
	 * 프로젝트의 어떤 method 에서든지 한번 담은 Session 정보는 전체 프로젝트의Controller method 에서 값을 참조 할수 있다 
	 */
