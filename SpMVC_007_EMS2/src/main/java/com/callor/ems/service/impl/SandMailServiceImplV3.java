...
@Slf4j
@Service(QualifyConfig.SREVICE.MAIL_V3)
public class SandMailServiceImplV3 extends SandMailServiceImplV1 {

	/*
	 * 프로젝트에서 src/main/resources 폴더에 저장된 파일들에 접근하기 위한 보조 도구
	 */
	protected final ResourceLoader loader;

	public SandMailServiceImplV3(JavaMailSender sender, ResourceLoader loader) {

		super(sender); // v1 생성자에게 sender보내기
		this.loader = loader;
	...
	public void sendMail(EmsVO emsVO,UserVO userVO) {
		...
		try {
			/*
			 * 두번째 전달 값 false : 순수 text만 전송가능 text/plan type 으로 전송 true : html 코드를 포함, 첨부파일도
			 * 포함 가능, text/html , multipart/form-data
			 */
