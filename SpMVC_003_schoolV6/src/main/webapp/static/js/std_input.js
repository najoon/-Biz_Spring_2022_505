document.addEventListener("DOMContentLoaded", () => {
  ...
  if (std_save) {
    std_save.addEventListener("click", () => {
      ...
      //js 코드를 사용하여 client Size validarion 을 수행
      ...
      if (st_dept.value === "") {
        alert("학과는 반드시 입력해야 한다");
        st_dept.focus();
        return false;
      }
      if (st_grade.value === "") {
        alert("학년은 반드시 입력해야 한다");
        st_grade.focus();
        return false;
      }
      /*
        ...
        parseInt() 함수
        문자열형 숫자를 문자열형 숫자로 바꾸는 함수

      문자열형 숫자를 숫자로 바꾸려고 할때 
      숫자형이 아닌 다른 데이터가 포함되어 숫자로 바꿀수 없을경우
      java 에서는 numberFormatException 이 발생되고 코드가 중단된다
      코드 중단을 방지하기위하여 try-catch 로 처리를 해주어야 한다

      그런데
       js 에서는 그러한 상황에서 exception  이 발생하는 대신
       nan(Not a Number : 숫자가 아니다) 이라는 겂을 return 한다

       js 에서 사용되는 특별한 데이터 들이 있다
