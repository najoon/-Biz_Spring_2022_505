...
public class AddressServiceImplV1 implements AddressService {
	...
	/*
	 * search 문자열이 null 일경우 전체 데이터를 조회할 수 있도록 만드는 코드를 Controller 에서 Service 로 이동
	 */
	@Override
	...
	/*
	 * 조건에 맞는 데이터를 가져와서 pagination 을 그리기위한 SearchPage 클래스의 데이터를 만들 것
	 */
	@Override
	public void searchAndPage(Model model, SearchPage searchPage) {
		// pagination 을 구현하기 위해서 전체 데이터 가져오기로
		// 개수를 임시로 세팅
		...
		// 검색어가 없이 요청이 될경우 SearchPage 객체의 search 값이 null 이 되어
		// 데이터가 검색이 되지 않는다
		...
		// 만약 전체 데이터가 37개 이고 한페이지에 10개를 보여준다면
		// finalPageNo 는 3 이된다
		long finalPageNo = totalCount / LIST_PER_PAGE;
		// 화면 하단에 페이지 번호를 클릭했을떼 전달되는 값
		...
		// 선택된 페이지 번호에 따라 화면하단에 번호 리스트를 유동적으로 보여주기 위한 연산
